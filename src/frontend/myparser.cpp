...
static ast::Type *p_Type() {
    ...
    Token t = lookahead(TokenType::INT);
    return new ast::IntType(t.loc);
}
...
static ast::Statement *p_Statement() {
    ...
    if (isFirst[SymbolType::Expression][next_token.type]) {
        ...
        // `If`/`LBRACE`/`SEMICOLON` else if (...) {} else if (...) {}
            ...
            Token t = lookahead(TokenType::SEMICOLON);
            return new ast::EmptyStmt(t.loc);
        } else {
            mind::err::issue(next_token.loc, new mind::err::SyntaxError(
...
static ast::ReturnStmt *p_Return() {
     ...
     **/
    Token t = lookahead(TokenType::RETURN);
    ast::Expr *expr = p_Expression();
    lookahead(TokenType::SEMICOLON);
    return new ast::ReturnStmt(expr, t.loc);
}
...
static ast::IfStmt *p_If() {
     ...
     **/
    Token t = lookahead(TokenType::IF);
    lookahead(TokenType::LPAREN);
    ast::Expr *cond = p_Expression();
    lookahead(TokenType::RPAREN);
    ast::Statement *body = p_Statement();
    ast::Statement *otherwise = new ast::EmptyStmt(t.loc);
    if (next_token.type == TokenType::ELSE) {
        lookahead(TokenType::ELSE);
        otherwise = p_Statement();
    }
    return new ast::IfStmt(cond, body, otherwise, t.loc);
}
...
static ast::Expr *p_Assignment() {
    ...
    if (next_token.type == TokenType::ASSIGN) {
         ...
         **/
        Token t = lookahead(TokenType::ASSIGN);
        ast::Expr *rhs = p_Expression();
        auto *var = dynamic_cast<ast::LvalueExpr *>(node);
        if (var == nullptr) {
            mind::err::issue(
                node->getLocation(),
                new mind::err::SyntaxError("expect lvalue expression"));
            return nullptr;
        }
        return new ast::AssignExpr(var->lvalue, rhs, t.loc);
    }
    return node;
}
...
static ast::Expr *p_Relational() {
    ...
    while (next_token.type == TokenType::LT ||
           next_token.type == TokenType::GT ||
           next_token.type == TokenType::LEQ ||
           next_token.type == TokenType::GEQ) {
