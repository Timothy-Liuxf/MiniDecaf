 ...
 */
template <typename _K, typename _V, typename _H> struct _HashIterator {
    // the value type
 ...
 */
template <typename _K, typename _V, typename _H = hash<_K>> class HashTable {
  public:
  ...
  private:
    ...
    typedef std::vector<_Node *, _Nodeptr_Alloc> vector_type;
 ...
 */
template <typename _K, typename _V, typename _H = hash<_K>> class HashMap {
  private:
    typedef HashTable<_K, _V, _H> _HashTable;
    _HashTable _ht;
